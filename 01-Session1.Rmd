...
Using the following measurements write some code that calculates the area of a square 
...
house_sale_prices <- list("brighton"=c(1000000,340000),"clayton"= c(190000,570000,4000000), "frankston"= c(2000000,5000000) )
...
Lists can be "indexed" by their names. They can also be indexed by position. Using a souble bracket returns the item inside the list, while using single bracket returns a list holding the elements that have been indexed. 
```{r}
...
If you might solve it by doing something like this
...
Some good places to look are:
 ...
 - https://datasetsearch.research.google.com/ for spefic dataset searches. 
